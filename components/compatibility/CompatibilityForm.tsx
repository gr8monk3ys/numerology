    ...
    <div>
      ...
      >
        <div className="grid gap-6 sm:grid-cols-2">
          {[
            { p: a, set: setA, title: "First person", slug: "a" },
            { p: b, set: setB, title: "Second person", slug: "b" },
          ].map(({ p, set, title, slug }, i) => (
            <div key={i} className="space-y-3">
              <h3 className="font-display text-lg text-gold-200">{title}</h3>
              <div>
                <label htmlFor={`cmp-${slug}-name`} className="label-text">
                  Name (optional)
                </label>
                <input
                  id={`cmp-${slug}-name`}
              ...
              <div>
                <label htmlFor={`cmp-${slug}-date`} className="label-text">
                  Date of birth
                </label>
                <input
                  id={`cmp-${slug}-date`}
                  ...
                  className={clsx("input-field date-field", !p.date && "is-empty")}
                  autoComplete="bday"
                />
              </div>
            </div>
        ...
        <div className="relative">
          <div className="flex items-center justify-center gap-6">
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-mystic-200/85">{aLabel}</p>
            </div>
            <span className="font-display text-2xl text-gold-300">✦</span>
            <div className="text-center">
              <NumberOrb value={result.b} size="lg" isMaster={isMaster(result.b)} />
              <p className="mt-2 text-sm text-mystic-200/85">{bLabel}</p>
            </div>
          </div>

          <div className="mx-auto mt-8 max-w-md">
            <div className="flex items-center justify-between text-sm">
              <span className="text-mystic-200/85">Harmony</span>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        {[
          ...
          <div key={label} className="glass p-6">
            ...
            {profile?.summary && (
              <p className="mt-3 text-sm text-mystic-200/85">{profile.summary}</p>
            )}
            {profile && (
...
function MatchRow({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="w-16 shrink-0 text-xs uppercase tracking-widest text-mystic-300/85">
        {label}
      </span>
